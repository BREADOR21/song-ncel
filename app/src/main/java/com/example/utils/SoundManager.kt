...
class SoundManager(private val context: Context) {
    ...
    private fun generateAndLoadSounds() {
        ...
        // Setup MediaPlayer with the game's theme track (zigdash_theme.mp3) on the Main thread
        CoroutineScope(Dispatchers.Main).launch {
            try {
                val mp = MediaPlayer.create(context, com.example.R.raw.zigdash_theme)
                if (mp != null) {
                    mp.isLooping = true
                    mp.setVolume(0f, 0f) // start silent, fade in on play
                    mediaPlayer = mp
                    ...
                    Log.e("SoundManager", "MediaPlayer.create returned null for R.raw.zigdash_theme")
                }
            } catch (e: Exception) {
    ...
    private var currentAppliedVolume = 0f

    /** Smoothly ramps the MediaPlayer volume from its current level to [target] over [durationMs]. */
    private fun fadeTo(target: Float, durationMs: Long, onComplete: (() -> Unit)? = null) {
        ...
        fadeJob = mainScope.launch {
            ...
            val start = currentAppliedVolume
            for (i in 1..steps) {
                val t = i / steps.toFloat()
                val v = (start + (target - start) * t).coerceIn(0f, 1f)
                try { mp.setVolume(v, v) } catch (_: Exception) {}
                currentAppliedVolume = v
    ...
    /** Adjusts playback tempo for an "adaptive music" feel as runs intensify. 1.0f = normal speed. */
    fun setMusicTempo(speed: Float) {
        val mp = mediaPlayer ?: return
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            try {
                val clamped = speed.coerceIn(0.85f, 1.25f)
                ...
                Log.e("SoundManager", "Error adjusting music tempo", e)
            }
        }
    }
