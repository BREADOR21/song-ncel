...
enum class PowerUpType { SHIELD, MAGNET }
...
class GameEngine(
    ...
    var onPowerUpCollected: ((PowerUpType) -> Unit)? = null

    // Shield state: absorbs one crash/fall
    var shieldActive = false

    // Magnet state: timed, pulls nearby coins
    var magnetActive = false
    var magnetTimeRemaining = 0f
    val magnetDuration = 8f   // seconds
    val magnetRadius = 220f   // virtual units
    ...
    fun reset(virtualHeight: Float) {
        ...
        speed = 380f
        pathWidth = 140f
        soundManager.setMusicTempo(1.0f)
        score = 0
        scoreXStart = ballX
        lastScoreX = ballX
        combo = 0
    ...
    fun revive() {
        ...
        particles.clear()
        floatingTexts.clear()
        powerUps.removeAll { it.position.x < ballX + 300f }
    ...
    private fun generateNextSegment() {
        ...
        // Power-up spawn (random, low probability)
        spawnPowerUp(lastPoint, dx, dy)
    }
    
    private fun spawnPowerUp(lastPoint: GamePoint, dx: Float, dy: Float) {
        // 8% chance per segment; never spawn on the first few segments
        if (points.size <= 10 || lastPoint.x < 2000f) return
        if (random.nextFloat() > 0.08f) return
        // Pick type: shield slightly rarer than magnet
        val type = if (random.nextFloat() < 0.45f) PowerUpType.SHIELD else PowerUpType.MAGNET
        // Center of segment, slightly offset perpendicular so it's clearly on-path
        val t = 0.5f
        val nx = -dy / kotlin.math.sqrt(dx * dx + dy * dy).coerceAtLeast(1f)
        val ny = dx / kotlin.math.sqrt(dx * dx + dy * dy).coerceAtLeast(1f)
        val side = if (random.nextBoolean()) 1f else -1f
        val px = lastPoint.x + dx * t + nx * 30f * side
        val py = lastPoint.y + dy * t + ny * 30f * side
        // Don't spawn too close to an existing obstacle or another power-up
        val tooClose = obstacles.any {
            val d = kotlin.math.sqrt((px - it.position.x).let { v -> v * v } + (py - it.position.y).let { v -> v * v })
            d < 80f
        } || powerUps.any {
            val d = kotlin.math.sqrt((px - it.position.x).let { v -> v * v } + (py - it.position.y).let { v -> v * v })
            d < 120f
        ...
        if (!tooClose) {
            powerUps.add(PowerUp(GamePoint(px, py), type))
        }
    }
    ...
    fun update(deltaTime: Float, isPlaying: Boolean = true) {
        ...
        if (playState == PlayState.RUNNING) {
            if (isPlaying) {
                ...
                // Adaptive music: tempo creeps up as the run intensifies, capped to stay musical
                val tempoT = (speedIncrease / 220f).coerceIn(0f, 1f)
                soundManager.setMusicTempo(1.0f + tempoT * 0.18f)
                ...
                // Magnet: pull nearby coins toward ball
                if (magnetActive) {
                    for (coin in coins) {
                        if (coin.collected) continue
                        val mdx = ballX - coin.position.x
                        val mdy = ballY - coin.position.y
                        val mdist = kotlin.math.sqrt(mdx * mdx + mdy * mdy)
                        if (mdist < magnetRadius && mdist > 1f) {
                            val pullStrength = (1f - mdist / magnetRadius) * 600f * dt
                            val nx = mdx / mdist
                            val ny = mdy / mdist
                            coin.position = GamePoint(
                                coin.position.x + nx * pullStrength,
                                coin.position.y + ny * pullStrength
                            )
                        }
                    }
                }
                ...
                while (puIterator.hasNext()) {
                    ...
                    val puDx = ballX - pu.position.x
                    val puDy = ballY - pu.position.y
                    val puDist = kotlin.math.sqrt(puDx * puDx + puDy * puDy)
                    if (puDist < (ballRadius + pu.radius + 16f)) {
                        pu.collected = true
                        when (pu.type) {
                            PowerUpType.SHIELD -> {
                                shieldActive = true
                                floatingTexts.add(FloatingText(
                                    ...
                                    vy = -170f, maxAge = 1.2f, isPerfect = true
                                ))
                            }
                            PowerUpType.MAGNET -> {
                                magnetActive = true
                        ...
                        for (i in 0 until 20) {
                            val angle = random.nextFloat() * 2f * Math.PI.toFloat()
                            val pSpeed = 120f + random.nextFloat() * 200f
                            particles.add(GameParticle(
                                x = pu.position.x, y = pu.position.y,
                                vx = kotlin.math.cos(angle) * pSpeed,
                                vy = kotlin.math.sin(angle) * pSpeed,
                                color = if (random.nextBoolean()) puColor else Color.White,
                                size = 5f + random.nextFloat() * 6f,
                                alpha = 1f, decay = 1.5f, isSparkle = true
                            ))
                        }
                        tapRings.add(TapRing(
                            x = pu.position.x, y = pu.position.y,
                            radius = pu.radius, maxRadius = pu.radius * 8f,
                            color = puColor
                        ))
                        soundManager.playScoreSound()
                        vibrationManager.vibrateTap()
                        onPowerUpCollected?.invoke(pu.type)
                        puIterator.remove()
                    }
                }
                ...
                // Cleanup obstacles, coins and power-ups too far behind
                obstacles.removeAll { it.position.x < ballX - 600f }
                coins.removeAll { it.position.x < ballX - 600f }
                powerUps.removeAll { it.position.x < ballX - 600f }
                
                // Check path collision
                val isOnPath = checkBallOnPath()
                if (!isOnPath) {
                    if (shieldActive) {
                        // Shield absorbs the fall — bounce back to path center
                        shieldActive = false
                        safeTimeRemaining = 1.5f
                        // Snap back toward last safe path node
                        val safeNode = points.lastOrNull { it.x <= ballX } ?: points.firstOrNull()
                        if (safeNode != null) { ballX = safeNode.x; ballY = safeNode.y }
                        cameraX = ballX; cameraY = ballY
                        trail.clear()
    ...
    private fun checkObstacleCollision() {
        ...
        for (obstacle in obstacles) {
            ...
            if (dist < (ballRadius + obstacle.radius)) {
                if (shieldActive) {
                    // Shield absorbs the obstacle hit
                    shieldActive = false
                    safeTimeRemaining = 1.0f
                    triggerShieldBreakEffect()
                    obstacle.passed = true // mark as passed so it doesn't re-trigger
                } else {
                    // No shield — crash
                    triggerExplosion()
                    playState = PlayState.CRASHED
    ...
    private fun triggerShieldBreakEffect() {
        // Big electric-blue burst when shield shatters
        ...
        floatingTexts.add(FloatingText(
            ...
            text = "🛡 SHIELD BROKEN",
            color = Color(0xFF00BFFF),
            vy = -150f, maxAge = 1.4f, isPerfect = true
        ))
        tapRings.add(TapRing(x = ballX, y = ballY, radius = ballRadius, maxRadius = ballRadius * 9f, color = Color(0xFF00BFFF)))
        for (i in 0 until 30) {
            val angle = random.nextFloat() * 2f * Math.PI.toFloat()
            val spd = 160f + random.nextFloat() * 260f
            particles.add(GameParticle(
                x = ballX, y = ballY,
                vx = kotlin.math.cos(angle) * spd,
                vy = kotlin.math.sin(angle) * spd,
                color = if (random.nextBoolean()) Color(0xFF00BFFF) else Color.White,
                size = 4f + random.nextFloat() * 7f,
                alpha = 1f, decay = 1.8f, isSparkle = true
            ))
        }
    }
